...
name: Hucom-System 瑠璃
slug: hucom-system
source-url: https://hucomsystem.com/
created: 2026-08-17
updated: 2026-08-18
notes: ykm4 ベースから派生。自社サイトの瑠璃 #1e50a2。link は accent の青と紛れるためティール #0f766e へ変更
-->
# Style Guide
...
Current skin is the Ykm4 editorial palette — warm off-white paper, dark teal-grey ink, lapis-blue accent, grey-green muted — derived 2026-08-17 from the `ykm4` base profile (accent swapped to visually identify the hucom-system Claude profile). Swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
| `accent` | Focal / 1–2 max per diagram | `#1e50a2` (accent) | `#7da4de` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(30,80,162,0.08)` | `rgba(125,164,222,0.10)` |
| `link` | HTTP/API calls, external arrows | `#0f766e` | `#5eb8ae` |

> **Brand palette source:** this skin maps to Ykm4's design system (`~/.config/claude/_shared/rules/design-system.md`) — `ink #1c2b2d`, `ink-soft #52625f`, `ink-faint #636f6b`, `paper #fbfaf6`, `sunk #f2f1ea`, `rule #d8ded4`, `accent #1e50a2` (dark variants from the same table). The `rule-solid` and `accent-tint` tokens are derived (ink/accent at opacity); `link` uses teal #0f766e (dark #5eb8ae) because this profile’s accent is itself blue (#1e50a2) and the shipped link blue would be indistinguishable from it. The accent replaces the base deep green #2f6f4e so diagrams are identifiable per Claude profile. AA contrast on light paper: ink 14.0, muted 6.1, soft 5.0, accent 7.4.
...
Any `rgba(28,43,45, X)` in light becomes `rgba(230,235,228, X)` in dark. Same opacities, RGB flipped. The accent gets a slight hue-shift brighter (`#1e50a2` → `#7da4de`) to read on dark paper.
