...
- gcloud はローカル CLI を直接使い、`--project=hucomsystem-gws`（プロジェクトに属さないコマンドは `--billing-project=hucomsystem-gws`）を付けて実行する。認証は `mise run google:gcloud-login hucom-system` で行う。
